...
class RoleModule extends Controller
{
  ...
  {    //Validate is ajax
    if ($this->request->isAJAX()) {
      ...
      if ($this->data[$this->model] = $this->roleModuleModel->where($this->primaryKey, $id)->first()) {
        $this->data['message'] = 'success';
        $this->data['response'] = ResponseInterface::HTTP_OK;
        $this->data['csrf'] = csrf_hash();
      } else {
    ...
    {    //Validate is ajax
      if ($this->request->isAJAX()) {
        //Select user status model 
        if ($this->data[$this->model] = $this->roleModuleModel->sp_role_module_id($id)) {
          $this->data['message'] = 'success';
          $this->data['response'] = ResponseInterface::HTTP_OK;
          $this->data['csrf'] = csrf_hash();
        } else {
      ...
      {    //Validate is ajax
        if ($this->request->isAJAX()) {
          //Select user status model 
          if ($this->data[$this->model] = $this->roleModuleModel->sp_permissions_module_id($id)) {
            $this->data['message'] = 'success';
            $this->data['response'] = ResponseInterface::HTTP_OK;
            $this->data['csrf'] = csrf_hash();
          } else {

...
class ProgramsGroups extends Controller
{
  ...
  public function getInstructorGroups($id)
  {
    ...
    if ($this->request->isAJAX()) {
      ...
      if ($this->data[$this->model] = $this->instructorModel->sp_instructs_no_group($id)) {
  ...
  public function getInstructorNoGroups()
  {
    ...
    if ($this->request->isAJAX()) {
      //Select instructor  model 
      if ($data[$this->model] = $this->instructorModel->sp_instructs_group()) {
        $data['message'] = 'success';
        $data['response'] = ResponseInterface::HTTP_OK;
        $data['csrf'] = csrf_hash();
      } else {
        $data['message'] = 'Error get Instructors Group';
        $data['response'] = ResponseInterface::HTTP_NO_CONTENT;
        $data['data'] = '';
      }
    } else {

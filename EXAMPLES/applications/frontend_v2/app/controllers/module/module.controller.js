...
function createTable(data) {
  ...
  for (let i = 0; i < rowLong; i++) {
    ...
    let dataRow = `<tr>
      ...
      <td>${row.name}</td>
      <td>${row.route}</td>
      <td><i class="bi ${row.icon}"></i></td>
      <td>${row.description}</td>`;
      if (row.is_active == 1) {
        dataRow += `<td><input class="form-check-input" name="is_active" class="form-check-input" type="checkbox" checked role="switch" id="" disabled></td>`;
      } else {
        dataRow += `
        <td><input class="form-check-input" name="is_active" class="form-check-input" type="checkbox" role="switch"  id="" disabled></td>`; }
    dataRow += `
      <td>
      ...
      <button type="button" title="Button Delete" class="btn btn-danger" onclick="delete_(${row.id})"><i class='fas fa-trash' ></i></button> </td>`;
